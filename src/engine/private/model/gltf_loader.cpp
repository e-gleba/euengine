                       ...
                       aabb&                      bounds)
{
    ...
    // Load texcoords
    // GLTF uses V=0 at top (DirectX convention), but OpenGL/Vulkan expect V=0 at bottom
    // Since textures are flipped vertically when loaded, we need to flip V coordinate here
        ...
        uv_it != primitive.attributes.end())
    {
        ...
        fastgltf::iterateAccessorWithIndex<glm::vec2>(
            ...
            { mesh.vertices[base_vertex + idx].texcoord = glm::vec2(uv.x, 1.0f - uv.y); });
    }
        ...
        joints_it != primitive.attributes.end())
    {
        ...
        fastgltf::iterateAccessorWithIndex<glm::u16vec4>(
            ...
            {
                mesh.vertices[base_vertex + idx].joints = glm::uvec4(j.x, j.y, j.z, j.w);
            });
    }
                           ...
                           loaded_mesh& mesh)
{
    ...
    for (const auto& target : primitive.targets)
    {
        ...
        if (auto pos_it = std::ranges::find_if(target, 
        ...
        if (auto norm_it = std::ranges::find_if(target,
            [](const auto& attr) { return attr.name == "NORMAL"; });
            norm_it != target.end())
        {
            const auto& norm_accessor = asset.accessors[norm_it->accessorIndex];
...
/// Extract texture data from glTF image - handles both external files and embedded textures
    ...
    const std::filesystem::path& base_path)
{
    ...
    if (std::holds_alternative<fastgltf::sources::URI>(img.data))
    {
        ...
        if (uri_str.find("data:") == 0)
        {
            ...
            if (comma_pos != std::string::npos)
            {
                ...
                spdlog::warn("  embedded data URI texture (base64 decode not yet implemented)");
            }
        }
        else
        {
    ...
    {
        const auto& buffer_view = std::get<fastgltf::sources::BufferView>(img.data);
        result.is_embedded = true;
        ...
        if (buffer_view.bufferViewIndex < asset.bufferViews.size())
        {
            ...
            if (view.bufferIndex < asset.buffers.size())
            {
                    ...
                    if (offset + length <= array.bytes.size())
                    {
                        ...
                        for (std::size_t i = 0; i < length; ++i)
                        {
                            result.embedded_data.push_back(
                                static_cast<unsigned char>(array.bytes[offset + i]));
                        }
                        ...
                        spdlog::info("  embedded texture: {} bytes ({})",
                                     length,
                                     result.mime_type.empty() ? "unknown" : result.mime_type);
                        return result;
                    }
                }
                else if (std::holds_alternative<fastgltf::sources::URI>(buffer.data))
                {
                    ...
                    spdlog::warn("  texture from external buffer (not yet supported)");
                }
            }
        }
    }
...
void extract_textures_and_materials(
    const fastgltf::Asset& asset,
    const std::filesystem::path& base_path,
    ...
    std::vector<model_material>& materials)
{
    ...
    std::unordered_map<std::size_t, std::size_t> texture_index_map; // glTF texture index -> our texture index
    ...
    for (std::size_t mat_idx = 0; mat_idx < asset.materials.size(); ++mat_idx)
    {
        ...
        mat.name = mat_name.empty() 
            ? std::format("Material_{}", mat_idx) 
            : mat_name;
        ...
        if (pbr.baseColorTexture.has_value())
        {
            ...
            if (tex_idx < asset.textures.size())
            {
                ...
                if (tex.imageIndex.has_value())
                {
                    ...
                    {
                        auto model_tex = extract_texture(asset, *tex.imageIndex, texture_type::base_color, base_path);
                        if (!model_tex.path.empty() || !model_tex.embedded_data.empty())
                        {
        ...
        if (pbr.metallicRoughnessTexture.has_value())
        {
            ...
            if (tex_idx < asset.textures.size())
            {
                ...
                if (tex.imageIndex.has_value())
                {
                    ...
                    if (it == texture_index_map.end())
                    {
                        auto model_tex = extract_texture(asset, *tex.imageIndex, texture_type::metallic_roughness, base_path);
                        if (!model_tex.path.empty() || !model_tex.embedded_data.empty())
                        {
                            mat.metallic_roughness_texture_index = textures.size();
                            texture_index_map[tex_idx] = textures.size();
                            textures.push_back(std::move(model_tex));
                        }
                    }
        ...
        if (gltf_mat.normalTexture.has_value())
        {
            ...
            if (tex_idx < asset.textures.size())
            {
                ...
                if (tex.imageIndex.has_value())
                {
                    ...
                    if (it == texture_index_map.end())
                    {
                        auto model_tex = extract_texture(asset, *tex.imageIndex, texture_type::normal, base_path);
                        if (!model_tex.path.empty() || !model_tex.embedded_data.empty())
                        {
        ...
        if (gltf_mat.occlusionTexture.has_value())
        {
            ...
            if (tex_idx < asset.textures.size())
            {
                ...
                if (tex.imageIndex.has_value())
                {
                    ...
                    if (it == texture_index_map.end())
                    {
                        auto model_tex = extract_texture(asset, *tex.imageIndex, texture_type::occlusion, base_path);
                        if (!model_tex.path.empty() || !model_tex.embedded_data.empty())
                        {
        ...
        if (gltf_mat.emissiveTexture.has_value())
        {
            ...
            if (tex_idx < asset.textures.size())
            {
                ...
                if (tex.imageIndex.has_value())
                {
                    ...
                    if (it == texture_index_map.end())
                    {
                        auto model_tex = extract_texture(asset, *tex.imageIndex, texture_type::emissive, base_path);
                        if (!model_tex.path.empty() || !model_tex.embedded_data.empty())
                        {
    ...
    // Fallback: if no materials found, try to find any texture file next to the model
    if (textures.empty())
    {
        ...
        if (textures.empty() && std::filesystem::exists(base_path.parent_path()))
        {
                        ...
                        std::vector<model_animation>& animations)
{
    ...
    for (std::size_t anim_idx = 0; anim_idx < asset.animations.size(); ++anim_idx)
    {
        ...
        anim.name = anim_name.empty() 
            ? std::format("Animation_{}", anim_idx)
            : anim_name;
        ...
        for (const auto& gltf_sampler : gltf_anim.samplers)
        {
            ...
            switch (gltf_sampler.interpolation)
            {
                ...
                case fastgltf::AnimationInterpolation::CubicSpline:
                    sampler.interpolation = animation_interpolation::cubic_spline;
                    break;
                default:
                    sampler.interpolation = animation_interpolation::linear;
                    break;
            ...
            {
                const auto& output_accessor = asset.accessors[gltf_sampler.outputAccessor];
                sampler.output.reserve(output_accessor.count);

                // Output can be vec3 (translation/scale) or vec4 (rotation quaternion/weights)
...
void extract_skins(const fastgltf::Asset& asset,
                   std::vector<model_skin>& skins)
{
    ...
    for (std::size_t skin_idx = 0; skin_idx < asset.skins.size(); ++skin_idx)
    {
        ...
        skin.name = skin_name.empty()
            ? std::format("Skin_{}", skin_idx)
            : skin_name;
        ...
        for (std::size_t joint_idx : gltf_skin.joints)
        {
            ...
            if (joint_idx < asset.nodes.size())
            {
                ...
                j.name = joint_name.empty()
                    ? std::format("Joint_{}", joint_idx)
                    : joint_name;
            }
        ...
        {
            const auto& ibm_accessor = asset.accessors[*gltf_skin.inverseBindMatrices];
        ...
        else
        {
            ...
            skin.inverse_bind_matrices.resize(skin.joints.size(), glm::mat4(1.0f));
        }
                         ...
                         std::vector<std::size_t>& root_nodes)
{
    ...
    for (std::size_t node_idx = 0; node_idx < asset.nodes.size(); ++node_idx)
    {
        ...
        node.name = node_name.empty()
            ? std::format("Node_{}", node_idx)
            : node_name;
        ...
        node.children.assign(gltf_node.children.begin(), gltf_node.children.end());
...
load_result gltf_loader::load(const std::filesystem::path& path) const
{
    ...
    extract_textures_and_materials(asset, path, model.textures, model.materials);
    ...
    for (std::size_t node_idx = 0; node_idx < asset.nodes.size(); ++node_idx)
    {
        ...
        // Process each primitive as a separate mesh (or combine if same material)
        for (const auto& primitive : gltf_mesh.primitives)
        {
            ...
            process_primitive(asset, primitive, world_transform, mesh, model.bounds);
    ...
    spdlog::info("=> model (gltf): {} ({} mesh nodes, {} total meshes, {} materials, {} textures, {} animations)",
    ...
    if (!model.meshes.empty())
    {
        ...
        spdlog::info("  => total vertices: {}, total indices: {}", 
                     total_verts, model.meshes[0].indices.size());
    }
